...
use Rbac\Permission\Requests\AdminUser\UpdateAdminUserRequest;
class AdminUserUpdateService
{
    ...
    public function update($id)
    {
        ...
        //create里有需要保存的数据
        //roleIds保存到关系表里
        $user = $this->repository->update($this->request->except('roleIds'),$id); //except releIds 排除在外
        $user->roles()->detach(); //对原有权限进行解绑 
        $user->roles()->attach($this->request->input('roleIds')); //完成用户绑定， 用户和角色的绑定
        return $user; 
        // 控制器最终调用这个update方法我们需要知道保存用户是什么样子的最终把用户交给Transformer
